...
public partial struct NpcAttackJob : IJobEntity
{
        ...
        [ChunkIndexInQuery]int sortKey)
    {
        ...
        if (!attackCooldown.GetDataAtTick(CurrentTick, out var cooldownExperationTick))
        {
            cooldownExperationTick.Value = NetworkTick.Invalid;
        } 

        var canAttack = !cooldownExperationTick.Value.IsValid || CurrentTick.IsNewerThan(cooldownExperationTick.Value);
